...
if($args.Count -ne 3){
        Write-Host "please specify all required variables"
        exit 1
}
...
do{
  $webui_ip = $(kubectl get services --namespace $namespace publisher-webui --output jsonpath='{.status.loadBalancer.ingress[0].ip}')
  Write-Host "Waiting for Web UI service to be created..."
  Start-Sleep -Seconds 10
}while($null -eq $webui_ip)
Write-Host "$webui_ip"
